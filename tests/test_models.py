...
class TestSimpleLinearRegression(unittest.TestCase):
    ...
    def test_nn(self):
        ...
        dense1 = Dense(activation=ReLU, input_shape=(1), neurons=3)


        dense2 = Dense(activation=ReLU, input_shape=(1,3), neurons=2)


        dense3 = Dense(input_shape=(1,2), neurons=1)
        ...
        data_len = 100
        data_step = 10
        x = np.array([((i-50)) for i in range(0,data_len*data_step,data_step)]).reshape(data_len,1)
        ...
        model.fit(x=x, y=y,epochs=500,lr=0.001, batch_size=10)


        while True:
            print('En farenheits: ', model.predict(int(input('Introduce un numero en grados celcius: '))))
